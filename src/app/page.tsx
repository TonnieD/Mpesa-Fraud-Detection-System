...
import { Upload, FileDown, RefreshCw, AlertTriangle, CheckCircle } from "lucide-react";
...
export default function Dashboard() {
  ...
  const [fileName, setFileName] = useState("Feature_engineered.csv");
  ...
  const loadDefaultData = async () => {
    try {
      setLoading(true);
      setError(null);
      setFileName("Feature_engineered.csv");
      setLoadingProgress("Fetching default database (28 MB)...");
      
      const response = await fetch("/data/feature_engineered.csv");
      if (!response.ok) {
        throw new Error("Failed to load default dataset from public assets.");
      }
  ...
  // Perform aggregations only if data is loaded
  ...
  // Chart 2: Amount Distribution (Right skewed bar chart)
  const amountBuckets = [
  ...
  // 6 buckets (0-10%, 10-25%, 25-50%, 50-100%, 100-200%, >200%)
  const drainBuckets = [
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between border-b border-gray-200 pb-6 gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Exploratory analysis of M-Pesa transaction patterns and fraud signals
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div className="text-right hidden sm:block">
            ...
            <p className="text-sm font-medium text-[#1B5E20]">{fileName}</p>
          </div>
          
          <button
            onClick={loadDefaultData}
            title="Reload default dataset"
            className="p-3 bg-white hover:bg-gray-50 border border-gray-200 text-gray-700 rounded-xl transition-all shadow-sm flex items-center justify-center hover:border-gray-300"
          >
      ...
      {error && (
        <div className="bg-red-50 border-l-4 border-[#DC2626] p-4 rounded-xl flex items-start gap-3 animate-fade-in shadow-sm">
          ...
          <div>
            <h3 className="font-semibold text-red-800">Dataset Validation Failed</h3>
      ...
      {loading ? (
        <div className="space-y-8">
          <div className="bg-white border border-gray-100 p-8 rounded-2xl flex flex-col items-center justify-center text-center shadow-sm">
            ...
            <p className="text-sm text-gray-500 mt-1">Processing datasets locally in your browser sandboxed workspace</p>
          </div>
          <LoadingSkeleton />
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className="text-xs text-gray-400 mt-2 font-medium">Scored database footprint</p>
            </div>
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              ...
              <p className="text-xs text-gray-400 mt-2 font-medium">Mean individual client ticket size</p>
            </div>
          </div>
                  ...
                  <BarChart data={drainChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    <Bar dataKey="fraudRate" radius={[4, 4, 0, 0]}>
                      {drainChartData.map((entry, index) => {
                        ...
                        if (entry.fraudRate > 10) color = "#DC2626"; // high risk red
                        else if (entry.fraudRate > 1) color = "#F59E0B"; // medium risk amber
